...
const sendEmail = async (to, subject, text, timeoutMs = 10000) => {
  const emailPromise = (async () => {
    try {
      // Use Resend API (HTTP-based, works on Render)
      ...
      // Note: Gmail SMTP doesn't work on Render (SMTP ports blocked)
      else if (process.env.EMAIL_USER && process.env.EMAIL_PASS) {
        console.warn('⚠️ Gmail SMTP may not work on Render (SMTP ports blocked). Use Resend API instead.');
        ...
        console.error('❌ Email configuration missing - no RESEND_API_KEY or EMAIL_USER/EMAIL_PASS provided');
