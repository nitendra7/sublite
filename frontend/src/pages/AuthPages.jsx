...
export default function AuthPage({ isLogin = true }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isLogin) {
        ...
        // Password validation is already done above, but keep this as backup
        const signupPasswordError = validatePassword(formData.password);
  ...
  const handleResetPassword = async (e) => {
    ...
    if (passwordError) {
      setForgotResetError(passwordError);
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {isLogin && (
            <div className="flex justify-end mb-2">
              <button
                type="button"
                className="text-sm text-[#2bb6c4] hover:underline font-semibold focus:outline-none"
